...
function AdminLayout() {
  ...
  const NAV = [
    ...
    { to: "/admin/products", label: t("nav.menu"), icon: UtensilsCrossed },
    { to: "/admin/orders", label: t("nav.orders"), icon: ListOrdered },
    { to: "/admin/tables", label: t("nav.tables"), icon: QrCode },
    { to: "/admin/reservations", label: t("nav.reservations"), icon: CalendarCheck },
  ];
      ...
      <aside className="border-b hairline glass-strong lg:sticky lg:top-0 lg:h-screen lg:border-b-0 lg:border-r">
        <div className="flex items-center justify-between p-5 lg:block">
          ...
          <div className="flex flex-wrap items-center gap-2 lg:mt-6">
            <Link to="/kitchen" className="inline-flex items-center gap-2 rounded-full border hairline px-3 py-1.5 text-[10px] uppercase tracking-[0.2em] text-gold hover:bg-gold/10">
              <ChefHat className="h-3 w-3" /> {t("nav.kitchen")}
            </Link>
            <LanguageToggle />
          </div>
        </div>
                ...
                <li key={n.to}>
                  ...
                  >
                    ...
                    {n.to === "/admin/orders" && activeCount > 0 && (
                      <span className="ms-auto rounded-full bg-gold px-2 py-0.5 text-[10px] font-bold text-primary-foreground">{activeCount}</span>
                    )}
                  </Link>
                </li>
              );
        ...
        <div className="hidden p-5 lg:block lg:absolute lg:bottom-0 lg:left-0 lg:right-0">
          <div className="rounded-2xl border hairline bg-background/40 p-4 text-center">
            <div className="text-[10px] uppercase tracking-[0.3em] text-gold">{t("nav.loggedIn")}</div>
            ...
            >
              <LogOut className="h-3 w-3" /> {t("common.signOut")}
            </button>
          </div>
        </div>
      </aside>
