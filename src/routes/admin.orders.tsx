...
function OrdersPage() {
  ...
  return (
    <div className="space-y-6 p-5 md:p-8">
      <header>
        <div className="text-[10px] uppercase tracking-[0.3em] text-gold">{t("nav.orders")}</div>
        <h1 className="mt-2 font-display text-4xl">{t("orders.title")}</h1>
      </header>

      <div className="flex flex-wrap items-center gap-2">
        <div className="relative flex-1 min-w-[220px]">
          <Search className="absolute start-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder={t("orders.searchPh")} className="w-full rounded-full border border-border bg-input/60 py-2.5 ps-10 pe-4 text-sm focus:border-gold focus:outline-none focus:ring-2 focus:ring-ring" />
        </div>
        <div className="scrollbar-hide flex gap-1 overflow-x-auto">
          {STATUSES.map((s) => (
            <button key={s} onClick={() => setFilter(s)} className={`whitespace-nowrap rounded-full px-3.5 py-1.5 text-[11px] uppercase tracking-[0.2em] ${filter === s ? "bg-gold text-primary-foreground" : "border hairline text-muted-foreground hover:text-foreground"}`}>{t(`status.${s}`)}</button>
          ))}
        </div>
      </div>

      {isLoading && <div className="rounded-2xl glass p-16 text-center text-sm text-muted-foreground">{t("common.loading")}</div>}
          ...
          <article key={o.id} className="rounded-2xl glass p-5">
            <header className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="text-[10px] uppercase tracking-[0.25em] text-muted-foreground">{t("orders.order")}</div>
                  <div className="text-sm">#{o.id.slice(0, 6)}</div>
                </div>
                <div>
                  <div className="text-[10px] uppercase tracking-[0.25em] text-muted-foreground">{t("orders.placed")}</div>
                  <div className="text-sm">{timeSince(o.created_at!)} {t("orders.ago")}</div>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <select value={o.status} onChange={(e) => change(o.id, e.target.value as OrderStatus)} className={`rounded-full border-0 px-3 py-1.5 text-[11px] uppercase tracking-[0.2em] ${STYLE[o.status as OrderStatus]}`}>
                  {(["new", "preparing", "ready", "delivered", "cancelled"] as OrderStatus[]).map((s) => <option key={s} value={s}>{t(`status.${s}`)}</option>)}
                </select>
            ...
            <ul className="mt-4 grid gap-1 text-sm sm:grid-cols-2">
              {o.order_items.map((it) => (
                <li key={it.id} className="flex justify-between gap-3 rounded-lg bg-background/30 px-3 py-2">
                  <span className="truncate"><span className="text-gold">{it.qty}×</span> {it.name}{it.notes && <em className="ms-1 text-xs text-muted-foreground">— {it.notes}</em>}</span>
            ...
            <footer className="mt-4 flex items-center justify-between border-t hairline pt-3 text-xs text-muted-foreground">
              <span>{t("orders.subtotal")} {formatPrice(Number(o.subtotal))} • {t("orders.service")} {formatPrice(Number(o.service))} • {t("orders.tax")} {formatPrice(Number(o.tax))}</span>
              <span className="font-display text-base text-foreground">{formatPrice(Number(o.total))}</span>
            </footer>
          </article>
        ))}
        {!isLoading && filtered.length === 0 && (
          <div className="rounded-2xl glass p-16 text-center text-sm text-muted-foreground">{t("orders.noMatch")}</div>
        )}
      </div>
    </div>
  );
